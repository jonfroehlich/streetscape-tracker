...
reads them for the current AND older snapshots. The aggregate cities.json.gz
is regenerated either way, since the overview site reads coverage from it.
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "holds an implausible capture date (issue #213)",
    )
    ...
    for r in rows:
        ...
        n_implausible = implausible_capture_date_count(df, pd.Timestamp(run_date), r["provider"])
        if n_implausible:
    ...
    if json_repairs:
        ...
        print(
            ...
            f"{'will be' if args.regenerate_json else 'is NOT'} rebuilt"
            f"{'' if args.regenerate_json else ' (pass --regenerate-json)'}"
        )
