...
— a `[providers.kartaview]` block is refused rather than accepted, with `run-due`/`assess-city` exiting `USAGE_EXIT_CODE` while one exists, because dueness gates on `cities.enabled` alone and would queue all 1,144 cities (#248); its cost arms ARE wired (#238), so the estimate is the swept-circle lattice × the measured 1.80× and never the GSV grid formula, and the previous run's observed `runs.api_requests` outranks that geometry because only it sees a city calibrated to r=500;
