...
`max_cities_per_day` is 400 (2026-09-25; 40 from #304) with both reservations SET at 10, and `max_batch_hours` 12: the deadline is the governor, and the bracket on it is `TimeoutStopSec` < `max_batch_hours` < `TimeoutStartSec` (14 h) less the ~0.45 h bounded tail — past ~13.5 h the unit has to move first.
