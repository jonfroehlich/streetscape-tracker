...
**The default preset of a pivoted table is trimmed to the measure, not fixed** (`fitDefaultPreset`): three metric groups fit two providers and overflow by 135px at three, and the e2e gate that should have caught it was green against a fixture narrower than production.
The budget stands in for a WIDTH, so it is **per-page** — grid 8 leaves, streets 10 (`STREET_PRESET_LEAF_BUDGET`) — and a leaf is as wide as the PROVIDER NAME in its header, never as its values.
...
**`streets.html` goes further and names no Δ at all in its default** (#345), which is what buys a third metric group; and its `age` group is ordered AHEAD of `walked`, so a fourth provider costs the walk date rather than the imagery age.
Coverage and recency are one answer for a deployment question, and the age group is in no other streets preset — trimming it there was total, not a deferral (a gate asserting the table FITS stayed green throughout).
