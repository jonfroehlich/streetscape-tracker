...
**A cap is a SOFT ceiling and a capped night can end over budget** — so never write "the budget is never exceeded": a tile gates on `api_requests + reserved` (without the reservation every task the semaphore admits clears a stale check and the cap is overshot by `connection_limit − 1` *every* capped night), and what is deliberately left is retries by tiles already in flight, at most `connection_limit × (TILE_MAX_TRIES − 1)`.
That residue is **20 on a grid census and 200 on a Mapillary road walk**, never one number: `cli.py` forwards `--connection-limit` only on its `gsv` arm, so a Mapillary or Panoramax grid runs at the downloader's own default of 5 while a walk takes the scheduler's per-child 50.
Since 2026-09-21 that 50 is `[download].connection_limit` (100) divided across lanes and clamped at `MAX_PER_CHILD_CONNECTION_LIMIT`, not the configured value itself.
...
The reserve now **leads** the slate after the hoist rather than ending the window, because the end of the window is where a deadline-stopped night cuts.
...
A night's cap is therefore split three ways — `opt_in_cities_per_day`, then `refresh_slots` of the remainder, then stalest-first — and it is the SUM of the two that bounds how much of a night the plain queue still governs (20 reserved slots lead a prod night; the deadline ends it).
`max_cities_per_day` is 80 (2026-09-25, the first step of a STAGED raise from #304's 40 — Mapillary per-IP volume must step up in stages) with both reservations SET at 10, and `max_batch_hours` 12: the deadline is the governor, and the bracket on it is `TimeoutStopSec` < `max_batch_hours` < `TimeoutStartSec` (14 h) less the ~0.45 h bounded tail — past ~13.5 h the unit has to move first.
Channels run back-to-back, or concurrently in host-disjoint lanes when `[schedule].max_concurrent_channels` > 1 (code default 1, **prod 2 since 2026-09-21**; channels sharing a per-IP host never overlap, so the effective ceiling is 5 of 8 — but 3 is the most a city due on only the four default channels can use, and `[download].connection_limit` is DIVIDED across lanes, so it moves with the knob or GSV gets slower).
...
`RUN_FILENAME_RE` is a deliberately STRICTER subset of `naming.FILENAME_RE`, not a mirror: it validates a URL a stranger handed the browser, so it takes `.csv.gz` only, no path component, no `#`, and integer dimensions — a name Python parses that it rejects is not a bug.
What must hold is the OVERLAP, and it is pinned across the language boundary by `test_the_js_run_filename_regex_agrees_with_python`, which reads the regex out of the JS source: **any name JS resolves to a provider, `parse_filename` resolves to the SAME provider.**
**A pivoted default preset carries EVERY metric group and the table scrolls sideways when that overflows (#350)** — #334's leaf budget is retired, because the group it dropped from the end was always a DATE (`grid.html` "Last collected" from three providers, `streets.html` "Walked" at four) and a collection date is not optional.
**There is no width to find**: a leaf is as wide as the PROVIDER NAME in its header, never as its values (GSV 95px → Panoramax 113px measured live at four), so compacting cells buys nothing and the "Walked" group alone costs 375px against a container the two other groups already fill exactly.
**The DOCUMENT must still never scroll sideways** — the width stays inside `.streets-table-wrap`, which needs its `position: relative` or the header's absolutely-positioned `.visually-hidden` span escapes and drags the page along; `driving.html` keeps the strict fits-its-container gate, having no provider fan-out to grow.
**The city column is pinned** (`position: sticky; left: 0`), or a scrolled row has no name — and it needs an opaque background, the row hover repainted on it, and `box-shadow` rather than `border-right`, since `border-collapse: collapse` paints a sticky cell's borders in the grid it left behind.
**Neither default names a Δ leaf**, on the `isGroupDelta` flag rather than the "Δ" glyph — not for width, but because a Δ compares two named providers while a group carries every one of them; `grid.html` has to say so explicitly, or retiring the trim silently hands back three columns it has not shown since #334.
**A default preset's title is assembled from `titleParts` rather than spelled** (`withPresetTitle`), or it promises a column that is not there — the same way #295's group titles enumerated providers and went stale; clause order matches the order the HEADER renders (the column registry's), not the preset's.
Two #334 lessons outlive its rule, both about tests: a layout gate is only as wide as its fixture — prod ran a provider ahead of it twice, so `tests/test_e2e_fixture.py` now fails in the FAST suite unless the committed fixture carries a run and a walk for every `KNOWN_PROVIDERS` entry on one city, omissions named with a reason in `build_fixture.FIXTURE_OMITTED_PROVIDERS` (#354) — and **"fits" is not "says anything"** — that gate stayed green the whole time a date group was missing, because a trimmed table fits by construction.
