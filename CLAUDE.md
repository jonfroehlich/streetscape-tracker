...
**A cap is a SOFT ceiling and a capped night can end over budget** — so never write "the budget is never exceeded": a tile gates on `api_requests + reserved` (without the reservation every task the semaphore admits clears a stale check and the cap is overshot by `connection_limit − 1` *every* capped night), and what is deliberately left is retries by tiles already in flight, at most `connection_limit × (TILE_MAX_TRIES − 1)` — **200** at the 50 sockets a prod CHILD gets, never the 20 the argparse default of 5 implies — and since 2026-09-21 that 50 is NOT `[download].connection_limit` (100), which the scheduler divides across lanes and clamps per child at `MAX_PER_CHILD_CONNECTION_LIMIT`.
