...
The launch floor and the pace a cap is sized against are **per provider, in one row each** (`_CRAWL_PRICING`) — sizing a tile census's cap from the radius sweep's constants under-prices it fourfold, and a cap under the floor skips the city rather than slowing it.
