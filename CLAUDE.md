...
The default `--provider both` requires both keys up-front (fail-fast so the series can't drift); a single-provider run needs only its own key.
...
— a `[providers.kartaview]` block is refused rather than accepted, with `run-due`/`assess-city` exiting `USAGE_EXIT_CODE` while one exists, since three arms in `scheduler.py` are fail-open and would price and time it as a GSV grid (#248);
