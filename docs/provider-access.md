...
**The reset test is fail-CLOSED and must stay so** — `download_common.overpass_serving` returns True only for an HTTP 200 from `/status` carrying a parseable slots line (`N slots available now` or `Slot available after:`) or the `Rate limit: 0` line an unlimited instance answers instead (the shape of the mirrors `OVERPASS_URL` is pointed at during an incident, on which the breaker could otherwise never clear), and False for unreachable, a timeout, any non-200 and any unfamiliar 200 body.
It runs on the scheduler's main thread and outside `host_lock(HOST_OVERPASS)` — the parent must never hold a host lock (#208), `/status` is unmetered — and holds the launch pass for at most its 15 s timeout, at most four times a night.
...
The re-check carries its own copy of the endpoint and identity (`DEFAULT_OVERPASS_URL`, `OVERPASS_USER_AGENT`, `OVERPASS_REFERER`), because the scheduler must not import osmnx; a test pins them equal to what osmnx is told, and the `OVERPASS_URL` mirror override reaches both.
