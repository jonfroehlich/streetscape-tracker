...
And **the stranding is named**: a walk an unavailable host cost a city — its own refused child, a breaker skip, or a *busy* exit (another local process held the lock; the breaker does not trip, but the city is stranded all the same) — whose grid sibling landed tonight is recorded on the breaker, counted on the `Done:` line, and listed in the alert beside the `run-due --provider <walk> --limit N` that walks it by hand; those walks will carry a later date than the grid runs, so the cities stay un-paired either way.
