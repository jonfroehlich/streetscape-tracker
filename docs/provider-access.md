...
Overpass turned out to be different.
The retained scheduler logs (from 2026-08-28) hold exactly two Overpass refusals, 2026-09-13 (07:20 PDT, the night's *first* network download) and 2026-09-15 (02:50, the fifth), and both cleared within about an hour: `/status` answered 200 with 2 slots by 08:04, and an HTTP response was back by 06:44.
Neither was our volume — real network downloads per night ran 27 / 20 / 14 / 1 / 27 / 5 over 09-10..09-15, far under Overpass's ~100/day guidance, and nothing but the scheduler and its child was touching Overpass on makelab2 — and *why* a low-volume client is refused is still not known.
What the latch cost was not the skipped walks but the pairing: each skipped city's GSV **grid** run still succeeded and advanced its clock, so the city left the night with a grid snapshot and no walk, not gsv-due for ~83 days, with `consecutive_failures` at 0 and no alert naming it.
Twenty cities in two nights, every one of them without a frozen GraphML — so every one could have been walked without touching Overpass had its network been fetched ahead of time.
`HostBreaker` (in `scheduler.py`) therefore separates *refused tonight* (the set, monotone — it still drives the alert and the nonzero exit) from *latched now*, and re-checks a latched host at submit time when a launch actually wants it: one request, at least `HOST_RECHECK_COOLDOWN_S` (45 min) after the trip or the last failed re-check, at most `HOST_RECHECKS_PER_NIGHT` (4) a night — 45 min sits inside the ~1 h both refusals took to clear, and four bounds a night's re-checks to the first three hours after a trip.
A host only gets a re-check if `HOST_RECHECKS` names a **positive** reset test for it, and the table holds Overpass alone: Mapillary, KartaView and Panoramax keep the original all-night latch by decision, not omission.
**The reset test is fail-CLOSED and must stay so, and since #356 it asks `/api/interpreter`, not `/status`** — `download_common.overpass_serving` POSTs `OVERPASS_PROBE_QUERY` (`[out:json][timeout:5][maxsize:1048576];node(1);out ids;make probe nonce="…";out;`) the way osmnx does, with osmnx's exact headers, and returns True only for an HTTP 200 whose JSON carries no `remark` and does carry the `probe` element echoing that call's random nonce.
Everything else is False: connection refused, a timeout, 429, 403, 406, any 5xx, a runtime-error `remark` (which Overpass serves under a 200), a non-JSON or unfamiliar body, a missing or stale nonce.
It runs on the scheduler's main thread and outside `host_lock(HOST_OVERPASS)` — the parent must never hold a host lock (#208) — and holds the launch pass for the probe's duration, normally bounded by `OVERPASS_PROBE_TIMEOUT_S` (25 s: Overpass queues a request up to 15 s for a slot, then runs it up to the declared 5 s), at most four times a night.
That 25 s is `requests`' per-connect and per-read timeout rather than a total, and the DNS lookup ahead of it takes no timeout argument at all, so it is the usual bound and not a hard one — the same was true of the 15 s the `/status` version quoted.

**Why the interpreter (issue #356, 2026-09-21).**
The first version read `/status` and required a slots line (or `Rate limit: 0`, for an unlimited mirror).
On 2026-09-21 it was refused at 06:07 (`Errno 111`), re-checked at 06:53 and cleared, was refused again at 06:57 — **4 minutes later** — cleared again at 07:43, and was refused again at 08:22, 39 minutes later: 41 launches skipped and 19 cities stranded, with `max_concurrent_channels = 1`, so one Overpass talker against a 2-slot allowance.
The predicate was correct as written; it measured the wrong thing.
`/status` is an unmetered code path separate from the query engine, and `overpass-api.de` is more than one backend (two IPv6 addresses in DNS plus an IPv4 announcing `lambert`, measured from makelab2 that night), so a clean status was never evidence that a query would be served.
The probe now exercises the path a walk takes — a slot from the per-IP dispatcher and a database read — and the nonce makes its answer unforgeable by anything that is not an interpreter running *this* query (a captive portal, a cached body, an error page under a 200).
It reads no slots line as well: a 200 from the interpreter means a slot *was* granted, which is stronger evidence than any status line, and a second request to a possibly different backend could only add a way to be wrong; the unlimited-mirror case needs no special handling any more, because a mirror answers the same query.
It is **metered**, unlike `/status`, so it is the cheapest query the engine can be asked: 5 s and 1 MiB declared instead of osmnx's 180 s / 512 MiB (Overpass admits a query by its declared needs, per the [overpass-doc "Commons" chapter](https://dev.overpass-api.de/overpass-doc/en/preface/commons.html)), one id lookup, and at most four a night against the [wiki's](https://wiki.openstreetmap.org/wiki/Overpass_API) "<100 queries/day" guidance for a regular application.
Node 1 is not required in the answer, so its deletion some day cannot fail the probe closed forever.
The shape was measured once from a laptop on 2026-09-22 (200, `application/json`, 278 bytes gzipped), never from a production host.
It also **connects the way osmnx does**: before every query osmnx's `_http._config_dns` resolves the host with `socket.gethostbyname` — one address, always IPv4 — and pins the request to it, and its own docstring gives the reason, that `overpass-api.de` fronts more than one server (it names gall and lambert) and a status check and a query can otherwise land on different ones.
...
It is deliberately *not* the fetch's advisory `_overpass_refusing` pre-flight, which is fail-OPEN by contract (a false alarm there skips a healthy city), because **the one confirmed abuse ban presented as connection refused**: on 2026-08-14 `overpass-api.de` firewalled makelab2 — TCP RST / connection refused on :443 while ICMP and other Hetzner hosts answered — for roughly 20 h, after the detached catch-up loop of #208.
A fail-open reset test reads that as "not refusing", clears the breaker into a live ban, and the next real fetch spends its whole retry window inside the host lock re-tripping it (~7.5 min under the default `[overpass]` policy, at most the 900 s deadline; it was 3–8 min before #357); whether those retries extend a ban is unknown, so it is not a rule to retry on "connection refused" and stop on 403/429/509.
**Duration is what separates a short refusal from a ban** (~1 h against ~20 h), and a spaced, capped, fail-closed re-check measures duration without guessing from the error type.
The re-check carries its own copy of the endpoint and identity (`DEFAULT_OVERPASS_URL`, `OVERPASS_USER_AGENT`, `OVERPASS_REFERER`, `OVERPASS_ACCEPT_LANGUAGE`), because the scheduler must not import osmnx; a test pins `overpass_headers()` equal to osmnx's own `_http._get_http_headers()`, and the `OVERPASS_URL` mirror override reaches both.
Two more things follow.
**A walk whose GraphML is already frozen is never breaker-skipped** — `fetch_graph` returns the cache before it takes the lock or probes, so that walk is not an Overpass request at all; only the Overpass entry is relaxed, keyed on the channel's configured `network_type`, and a walk whose census host is also latched still waits.
And **the stranding is named**: a walk an unavailable host cost a city — its own refused child, a breaker skip, or a *busy* exit (another local process held the lock; the breaker does not trip, but the city is stranded all the same) — whose grid sibling landed tonight is recorded on the breaker, counted on the `Done:` line, and listed in the alert beside the pasteable `run-due --provider <walks> --city <id>...` that walks exactly it (#362); started the same UTC day the walk keeps the grid run's date, later it carries a later one.
Prevention is `scripts/prefreeze_street_networks.py`: a daytime pass that freezes the next night's walk cities' networks serially and paced, through the same lock and probe a walk uses, so a mid-night refusal has nothing left to strand.
It shipped without a timer and so never ran until #355 added `streetscape-prefreeze.timer` (daily 15:00 Pacific, `--limit 40` against the policy's ~100 queries/day for a regular application); on 2026-09-21 a refusal stranded 19 cities, 18 of them with no frozen network.
Automatic mirror failover is still deliberately not done — `OVERPASS_URL` stays a manual, incident-time switch, since a mirror's own usage policy has to be read first and a genuinely banned client's traffic should not simply move.
...
Five things follow from that, and a sixth came later (#357).
...
(#357 later widened how LONG those two retry — see (6) — and deliberately not which errors do.)
...
**(4) A `/status` pre-flight** names a refusal in ~1 s instead of after the whole retry window (see (6)).
...
`OVERPASS_DEADLINE_S` is **derived** so it can't silently fall below the worst legitimate fetch: since #357 that is the longest retry window a policy may configure (`OVERPASS_RETRY_WINDOW_CEILING_S`, 600 s) plus one full final attempt — a request timeout plus 120 s of slack for osmnx's pre-request slot pause, which with `overpass_rate_limit = True` is the entire wait the server advertises — `600 + 180 + 120` = the same 900 it was when it read `3 * (OVERPASS_TIMEOUT_S + 120)`, "three attempts".
If it ever fires on healthy-but-busy nights, raise the slack — do not remove the bound, since unbounded is how the SIGKILL happens.
**(6) The retry window spans minutes, not seconds (issue #357).**
tenacity's `stop_after_attempt(3)` / `wait_exponential(min=4, max=10)` was ~12 s of backoff; when `/status` refused too, osmnx's 60 s fallback pause (`_get_overpass_pause` on a ConnectionError) sat in front of each attempt, so a refused walk died in ~3.2 min (2026-09-15), and when `/status` still answered — overpass-api.de is multi-backend, and on 2026-09-21 a clean `/status` preceded a refused `/interpreter` by minutes — it died in ~12 s.
Refusals that night flapped on a minutes scale (refused 06:07, serving by 06:53, refused 06:57, serving by 07:43, refused 08:22), so either window turned a flap into exit 76 and a latched breaker; 19 cities stranded.
The fetch now retries under `overpass_retry.OverpassRetryPolicy`, configured by `[overpass]` in the scheduler TOML and handed to every walk child (and the pre-freeze pass) as `--overpass-retry-*` flags: waits of **30, 60, 120, 240 s**, each stretched by up to 25% (upward only), giving up at 5 attempts or when the next wait would end past a **600 s** window, whichever is first — ~7.5–9.5 min in both shapes, 4 attempts with osmnx's pause and 5 without.
**30 s is a floor, enforced rather than defaulted**: the usage policy says "if you receive an HTTP error code such as 429 or 406, pause for 30 seconds before making a new request", and on the OSM forum (thread 146883, 2026-08-27) an Overpass operator answered a `Connection refused` report with "the IP had been blocked for ignoring the 429 return … If you get a 429, wait at least 30s before sending the next query" — so a config asking for less is refused, and a jitter that could shorten a wait below it is not offered.
The same operator (2026-09-01): "The IP bans are completely automatic and will disappear after a while" — consistent with duration being the discriminator, and silent on whether connection attempts during a ban extend it, which stays unknown.
Counted properly, an attempt is **two** connections rather than one — osmnx GETs `/status` before every POST (`_overpass_request`), and repeats the pair per uncached sub-query on a bbox it split — so the new schedule opens roughly 8–10 against a refusing host over ~7.5 min where the old one opened ~6 inside 12 s (or ~3 min).
The politeness conclusion is unchanged and rests on the SPACING rather than the count: consecutive POSTs are now ≥ 30 s apart against tenacity's 4–10 s, which is the thing the usage policy and the operator's reply both ask for.
What did NOT change, each on purpose: the predicate (a 406, a ban page or an empty bbox is still asked once), the classification (every retryable fault waits on the same schedule — no errno rule, per #341), osmnx's own 429/504 handling (a 55 s sleep and recursion, already past the 30 s floor, still bounded by the deadline), and the host lock, which is held across the whole window so no second local talker slips in between attempts.
The cost is wall clock in the walk's lane: an exhausted fetch holds the Overpass lock for its window, and the breaker trips at most `1 + HOST_RECHECKS_PER_NIGHT` = 5 times a night, so a night refusing throughout pays ≤ 5 windows (~40 min, bounded by five 900 s deadlines) against `max_batch_hours` = 12.
**The window a child actually gets is the smaller of the configured one and what its own timeout can hold** (`policy_for_child_timeout`), and that is not a refinement — without it the change would be a regression.
`city_timeout_seconds` clamps a late city's timeout down to what is left of the batch deadline, floored at `_MIN_CLAMPED_TIMEOUT_S` (300 s), which is BELOW the ~450 s a refusal now costs; a SIGKILL there carries no exit code, so it counts a `consecutive_failure` and the breaker never learns the host refused us — strictly worse than the ~12 s / ~3.2 min window this replaced, both of which always finished.
So the scheduler hands the child `timeout − 300 s (one final attempt) − 60 s (startup, imports, the lock and the pre-flight)`, and a single attempt when nothing is left.
This is a mitigation; the fix is to stop needing Overpass at night (#355's pre-freeze timer).
...
**Deliberately no new client-side pacer** — Overpass's own guideline is a daily volume ("fewer than 10,000 queries/day", and its docs add "divide by 100" for an app that queries regularly, i.e. under ~100/day), we issue far fewer than that — **27 / 20 / 14 / 1 / 27 / 5** real network downloads on 2026-09-10 through 09-15, counted in the per-attempt street logs (#339), because most walks load a frozen network from `data/osm_cache` and only a city's FIRST walk queries Overpass at all — and the ban came from concurrency (#208) and plausibly from (5), not from our steady-state rate.
...
What the cut does **not** move is the daily count: a night is capped at `[schedule].max_cities_per_day` (**40** in production since #304 on 2026-09-02, not the 20 this paragraph was first written against) however fast it runs, and the sleep only ever fired between cities that were actually attempted — so the gap is not what bounds Overpass, and at these rates neither is the cap: the binding term is how many of the night's cities are walking a network for the FIRST time.
That was measured rather than modelled, and it overturned the estimate this paragraph used to carry (#339): the 20 → 40 cap raise was expected to take nightly Overpass queries from ~20–40 to ~40–80, and the counted figures above are what actually happened.
So the quantity to re-check against the ~100/day reading is the share of a night's cities walking a network for the first time — not `max_cities_per_day`, and not `max_concurrent_channels`, which host affinity already holds to one Overpass talker whatever it says, and not the nightly slate, which is already ~940 due cities and has never been what bounds the count.
...
The one place this is not free is the pair Google meters per Cloud **project** rather than per IP: `gsv` and `gsv_streets` declare no host, so nothing serializes them, and running them concurrently presents **48k + 48k = 96k req/min from one IP** (it read 48k + 24k until `gsv_streets` was raised to its own project's newly approved ceiling on 2026-09-21).
That is safe **only if the two keys live in separate projects** — per project each is still the same ~80% of its own limit — and the projects are now recorded in `config/scheduler.makelab1.toml` beside `max_concurrent_channels` rather than being an unrecorded console check.
Re-confirm in the console if either key is rotated, and note the identification there rests on traffic signature rather than on reading key strings; a shared project must be fixed by splitting the keys rather than by declaring a fake host (a fake `CHANNEL_HOSTS` entry would couple the night-level breaker to something that is not a host refusal).
