...
A fail-open reset test reads that as "not refusing", clears the breaker into a live ban, and the next real fetch spends 3–8 minutes inside the host lock re-tripping it; whether those retries extend a ban is unknown, so it is not a rule to retry on "connection refused" and stop on 403/429/509.
