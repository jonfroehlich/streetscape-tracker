...
It runs on the scheduler's main thread and outside `host_lock(HOST_OVERPASS)` — the parent must never hold a host lock (#208) — and holds the launch pass for at most its 25 s timeout (`OVERPASS_PROBE_TIMEOUT_S`: Overpass queues a request up to 15 s for a slot, then runs it up to the declared 5 s), at most four times a night.
