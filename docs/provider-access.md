...
The same operator (2026-09-01): "The IP bans are completely automatic and will disappear after a while" — consistent with duration being the discriminator, and silent on whether connection attempts during a ban extend it, which stays unknown; the new schedule sends at most 5 connections over ~7.5 min where the old one sent 3 inside 12 s, so it is slower per unit time as well as longer.
