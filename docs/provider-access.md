...
It is a soft one: a capped crawl may end the night up to `connection_limit × (TILE_MAX_TRIES − 1)` requests over its budget — **200** at the 50 sockets a production *child* gets (since 2026-09-21 that is no longer `[download].connection_limit` itself, which is 100; the scheduler divides it across lanes and clamps the share at `MAX_PER_CHILD_CONNECTION_LIMIT`) — because tiles already in flight when the cap trips are allowed to finish their retries rather than being cancelled mid-attempt.
