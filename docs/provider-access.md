...
**The reset test is fail-CLOSED and must stay so** — `download_common.overpass_serving` returns True only for an HTTP 200 from `/status` carrying a parseable slots line (`N slots available now` or `Slot available after:`), and False for unreachable, a timeout, any non-200 and any unfamiliar 200 body.
...
And **the stranding is named**: a walk a refused host cost a city — its own refused child, or a breaker skip — whose grid sibling landed tonight is recorded on the breaker, counted on the `Done:` line, and listed in the alert beside the `run-due --provider <walk> --limit N` that walks it by hand; those walks will carry a later date than the grid runs, so the cities stay un-paired either way.
