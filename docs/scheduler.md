...
Above 5 buys nothing with today's eight channels, and above **3** buys nothing on the city shape prod actually collects: a city due on the four default-membership channels has a largest host-disjoint set of three (`gsv` + `gsv_streets` holding Overpass + `mapillary`), because the fourth, `mapillary_streets`, needs both Overpass and the tile CDN those two are already holding.
4 and 5 only start paying once `kartaview` and/or `panoramax` are due on the *same* city, which is why the ceiling and the useful value are different numbers.
Watch `MemoryPeak` after each night rather than pre-raising the unit's `MemoryHigh=40G`/`MemoryMax=48G` (raised from 20G/24G on 2026-09-02 under #305 and sized for three concurrent children; crossing `MemoryHigh` throttles *all* lanes into the documented reclaim stall), and if they must move, move High and Max together, keep the unit's quoted prose figures in step, and re-copy + `daemon-reload`.
...
`100 // 2 = 50` keeps every child on exactly today's socket count, so the knob buys overlap without paying for it out of GSV's throughput; going to 3 later needs **150**, not 100, for the same parity.
The memory gate was satisfied by measurement rather than argument: the `cgroup peak` line recorded every night from 2026-09-07 to 09-20 reads **1.30–7.89 GiB, 3–20% of `MemoryHigh`, throttled 0 times**.
