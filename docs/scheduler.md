...
**What this actually changed on the ground:** the permanent `est > budget` skip has never once fired for Mapillary (the largest grid is 870 tiles against a 3,500 budget), so the live win is the `used + est > budget` deferral becoming a capped launch — the end-of-night sliver that collected nothing from New York at 88% affordable.
