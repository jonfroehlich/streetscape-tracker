...
**Stranding is recorded after the city drains**: a walk a refused host cost the city — the refused child itself, or a breaker skip — whose grid sibling (`STREET_CHANNELS[walk]`) succeeded tonight leaves the city with a grid run and no walk and not due on the grid channel for ~83 days, so it is counted on the `Done:` line (`N city(ies) STRANDED un-walked by the breaker`), named in the alert with the `run-due --provider <walk> --limit N` recovery, and logged as a `STRANDED` warning per lost walk.
