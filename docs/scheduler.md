...
The "two settings that disagree" are named by the parser itself: its `error:` line is in the child-log tail the alert's recent log quotes, and the argv is on the `exited 2` line.
...
The one rejection the scheduler could plausibly provoke, `connection_limit > batch_size`, is now a clamp with a stderr warning in `cli.py` rather than a `parser.error`: `download_gsv` gathers at most `batch_size` requests per batch under a `TCPConnector(limit=connection_limit)`, so sockets past the batch never get work.
