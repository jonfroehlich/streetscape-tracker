...
Not *exactly* its budget, though — a capped crawl lets tiles already in flight finish their retries, so it can end up to `connection_limit × (TILE_MAX_TRIES − 1)` over — **200**, which is the per-CHILD 50 and not prod's `[download].connection_limit` of 100; the child's share is the config value divided by the lane count and clamped at `MAX_PER_CHILD_CONNECTION_LIMIT`.
...
**Going to 3 does *not* simply need 150 here** — an earlier draft said so in two files and that advice was unsafe: at one lane 150 exceeds `[download].batch_size` (100), and `cli.py` rejects `connection_limit > batch_size` with an argparse **exit 2** before any work, which the scheduler records as an ordinary child failure — five of which quarantine a city for a 90-day cycle.
Going further means raising `batch_size` first, then `MAX_PER_CHILD_CONNECTION_LIMIT` deliberately with a memory measurement behind it, and only then this number.
