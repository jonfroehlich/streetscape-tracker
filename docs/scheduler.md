...
The resolution happens against the run's *effective* cap, so `--limit` scales the split with the cap it overrides.
...
A derived reservation cannot reach the cap, so if it ever fires it means an operator set `opt_in_cities_per_day` equal to `max_cities_per_day` and re-created the unbounded hoist by hand — a legal configuration and a bad one.
