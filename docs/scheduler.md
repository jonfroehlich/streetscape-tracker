...
The original refusal objected to *invisibility* rather than to the operation ("a second less visible way to disable a city is how two operators disagree about why it stopped"), so allowing it is paid for on the visibility side: `status` prints `excluded` rather than `not enrolled` for an explicit `0`, and the membership footer prints a line for any default-membership channel carrying one.
