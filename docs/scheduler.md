...
The permanent `est > budget` arm is rarer and its reach depends on geometry that differs between catalogs; `docs/provider-access.md` records what was measured and what is still open there, and **do not quote a "largest grid" figure from a population of collected runs** — the gate skips exactly the expensive cities, so that number cannot answer how often it fired.
A night therefore spends its budget *exactly* rather than stopping short of it; the daily total is unchanged, and across nights a resumed city is cheaper, because its tiles are paid once instead of re-paid.
...
Inverting one channel's clock with the other's constants under-prices a Mapillary cap roughly fourfold at the shipped rates — and a cap below the launch floor does not slow a channel down, it **skips the city outright, nightly and silently**.
