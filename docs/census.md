...
The arm that hoists today is the **SIGKILL**, not the pause: `stop_reason` is set only by the `max_requests` guard and `_run_one_city` passes no `--kartaview-max-requests` (#273), so under `run-due` exit 83 cannot currently occur and the five-night bound is the live constraint rather than a residual one.
