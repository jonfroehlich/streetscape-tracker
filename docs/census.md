...
That is not a small residual, and it is not one number: a road walk runs at the per-child **50** so the figure is 49, while a *grid* census runs at **5** — `cli.py` forwards `--connection-limit` only on its `gsv` arm — so there the figure is 4.
Neither is "the argparse default": `cli.py`'s `--connection-limit` defaults to 50, and the 5 arrives from the downloader's own signature because the argument is never forwarded.
Since 2026-09-21 the walk's 50 is also not `[download].connection_limit` (100) — it is that value divided across lanes and clamped at `MAX_PER_CHILD_CONNECTION_LIMIT`.
...
The second also feeds `_tile_census_timeout_seconds`/`_kartaview_timeout_seconds`, where a 0 would collapse a child's timeout onto the flat floor.
...
That promotion is **bounded** by `[schedule].opt_in_cities_per_day` (#282): at most that many cities are moved, the rest keep their union position and wait for a later night.
The bound is also what lets it share a night with the refresh reserve (#308) instead of cancelling it — the hoist runs first and the reserve takes the slots it left, and `docs/scheduler.md` carries that arithmetic.
...
That foreclosure is retired: since #301 a **single-channel city IS expressible** — register it, `enroll-city --channel gsv --remove` (and `gsv_streets`) while it is still disabled, then enable it. What is still not expressible is a per-channel enable *date*: exclusion is a switch, not a schedule.
...
Since #273 a healthy sweep takes the **pause** rather than the SIGKILL: `_run_city_channels` hands the child a request cap, and sizes it to the wall clock as well as the ledger — `min(budget − used, what its own timeout can pace)` — so a sweep that would overrun exits 83, which records no failure and consumes no city-cap slot.
The clock term is not optional, and neither is the ledger one — which of the two binds moves with the config, which is why the cap is a `min` rather than whichever was smaller when it was written.
At the 10 h batch #273 was written against, prod's 16/min paced ~9,600 requests against a 10,000 budget, so a cap set to a fresh night's remainder was unreachable by arithmetic outright and the arm actually taken stayed the kill; at the 12 h batch (raised 2026-09-02) the same rate paces ~11,520 and the budget is the smaller term instead.
The five-night bound therefore no longer binds a paused city at all; what bounds it is `CHECKPOINT_MAX_AGE_S`, seven days from the checkpoint's first commit.
The SIGKILL arm remains for a child running **slower** than the assumed `rate × _SWEEP_ACHIEVED_RATE_FRACTION`, which no request cap can bound (only a clock inside the child could), and the five-night bound is that arm's.
...
The stop for that is now wired (**#273**): the launch site is `_run_city_channels`, which hands every resumable child #239's `--kartaview-max-requests` sized as `min(budget − used, what the child's own timeout can pace)`, so an overrun checkpoints the rest and exits 83 instead of spending past the ledger.
That is the plumbing half, which bounds an overrun on its own; **#274** is the policy half below that depends on it.
...
Reading the floor before `est` was the version of this that skipped the cheapest channel of the night for the cost of work it was not going to do.

**A checkpoint at the age wall is REFUSED, not annotated (#274 review).**
`CHECKPOINT_MAX_AGE_S` is measured from the checkpoint's first commit, and `load_checkpoint` discards an older one with a warning in the CHILD's log, after which the sweep re-commits on the same path with a fresh `created_at` — so a city whose cost exceeds ~7 nights of remainder re-sweeps from root 0 every week, forever, and nothing upstream sees it: a pause records no `consecutive_failure`, sets no `unhealthy` component and sends no email, and the discard night reads as ordinary progress because the state file it reports from has just been reset.
So `_sweep_launch_plan` reads the checkpoint BEFORE launching: within a night of the wall (`_CHECKPOINT_AGE_WALL_MARGIN_S`) and with more of the lattice left than tonight's cap can plausibly cover, it logs at WARNING, refuses to launch, and records a **real failure** for that (city, channel).
That last part is the fix rather than a side effect — the failure is what puts it in `attempted - succeeded`, which is what `_finish_batch` alerts on, and what the five-night backstop eventually quarantines the city with instead of discarding ~60k requests a cycle.
"Cannot finish tonight" is projected from `roots_done/root_count` against `est`, deliberately simply: root cells are not uniform and `est` is a floor, so a sweep 799 root cells into 800 is still resumed on its last night, which is the crawl the wall exists to protect.

**A road walk never sweeps a lattice its grid sibling is already sweeping.**
`kartaview_streets` ranks immediately after `kartaview` for the same city, and a PAUSED grid sweep leaves a *checkpoint*, not a cache entry — so the walk prices at full, and with the budget gates gone for resumable channels it would launch a second, independently checkpointed sweep of the same bbox against the same per-IP host.
The grid's census then lands in the cache, `reconcile_cache_hit` discards the walk's older checkpoint, and everything the walk spent is thrown away.
It defers instead: an INFO line and its own counter in the night's summary, no failure and no budget skip, until the grid completes and the walk prices at 0.
The pairing is read from `STREET_CHANNELS` — the table that already maps every walk to the provider whose census it reads — and never spelled into the launch site; and the deferral is conditional on the sibling being on TONIGHT's slate for that city, because waiting on a channel nothing is going to run is the same open-ended silence the age wall arm exists to end.

**`run-due --dry-run` makes the same decision, from the same helper.**
It used to print `OVER BUDGET (deferred)` whenever `est > budget_left`, which is the right question for an all-or-nothing channel and the wrong one for a sweep that pauses: the preview an operator reads before a night said a metro was deferred while the live path launched it capped.
Both now call `_sweep_launch_plan`, so the preview names the cap the child will actually get and applies the same three skips.
The one input it cannot share is the deadline clamp — a preview has no night in progress — so a previewed cap is an upper bound on what a city launched late in a real batch receives.
