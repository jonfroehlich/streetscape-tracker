...
- The KartaView request cap and the resumability branch (#273, #274), which are tested as the two halves they are.
  ...
  And the pause log's progress note, which reports `roots_done/root_count`, **warns** when the checkpoint is within a night of `CHECKPOINT_MAX_AGE_S` — the pathology no single night looks like, since the age wall discards a checkpoint mid-crawl and the city re-sweeps from scratch forever — and returns empty on an unreadable state file, because it runs inside a branch that has already decided the night was fine
