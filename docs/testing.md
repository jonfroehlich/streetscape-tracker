...
- The KartaView request cap and the resumability branch (#273, #274), which are tested as the two halves they are.
  ...
  The floor is straddled by **one request** rather than tested from one side: both budgets sit far under the city's estimate, so the over-budget arm would refuse either, and only `_MIN_SWEEP_LAUNCH_REQUESTS` can tell them apart — without the straddle the test passes with the branch reverted.
  ...
  And the pause log's progress note, which reports `roots_done/root_count` and returns empty on an unreadable state file, because it runs inside a branch that has already decided the night was fine.
...
- The three skips a resumable channel can take (#274 review), each pinned with the arm that must NOT fire beside it.
  ...
  The **calibration floor** not gating a free walk: a cached census (`est == 0`) with a remainder under the floor still launches, parametrized against the uncached half of the same city and budget, which is refused — without that half the test passes by making the floor unreachable.
