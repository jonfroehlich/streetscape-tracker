...
- The KartaView cost arms (issue #238), which are unusual in that **no runtime path reaches them** — `UNWIRED_CHANNELS` still refuses the config block, so the tests drive `SchedulerConfig` directly, which is exactly what the loader's refusal cannot reach.
  ...
  that a prior run's `runs.api_requests` *beats* the geometry, which is the only thing that sees a city calibrated to r=500 (Singapore: ~1,273 circles estimated against 9,974 spent);
