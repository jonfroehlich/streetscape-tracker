...
- Per-(city, channel) membership and the opt-in hoist (issue #248), which are tested as two separate halves because either one alone is a channel that is configured and collects nothing.
  ...
  Both unfinished-sweep arms are pinned, and since #273 both are live: the **deliberate pause** — now the common arm, because `_run_one_city` hands every sweep the night's remaining budget as `--kartaview-max-requests` — consumes no city-cap slot and records no failure, while the **timeout SIGKILL**, still reachable when a sweep runs out of wall clock rather than budget, counts a `consecutive_failure` and *does* consume a slot, yet still leads the next slate.
