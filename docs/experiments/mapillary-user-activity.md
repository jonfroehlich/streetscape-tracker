...
**Any number that does not is labelled *(hand)* — read in the 2026-09-22/23 hand probes — or *(uncommitted)* — from the exploratory run below, whose output was not recorded.**
...
Each was also given `--db` pointing at a laptop's **dev** catalog (recorded as `catalog_path`, a basename since `--normalize-metrics` rewrote the four records offline — the only change that pass makes), which matched the centroids to cities but holds 3 Mapillary runs in total — so every group reads `never_collected` there, and none of that is a statement about production.
...
- **`paging.next` works with `creator_username`**, as Mapillary's docs say it does only for that filter.
  Measured *(hand)*: following it from the 2026-09-15 window returned two further pages of 2,000 with **zero overlapping ids**, strictly newest-first (00:00:00 → 23:54:57.3, then 23:54:57.2 → 23:49:30.8, then 23:49:30.6 → 23:41:34), and the cursor URL carries no token.
  The committed 09-15 record corroborates it without the spans: 11 requests returned 21,933 images, which is 10 full pages and a partial eleventh.
...
- **2026-08-26 (solar), Spokane WA** — ≥ 8,808 at (47.661, −117.460), ≥ 6,782 at (47.689, −117.508), 326 at (47.610, −117.421), 84 at (47.701, −117.508); the by-hand ~47.717, −117.485 falls in that last group's cell and 47.610, −117.421 in the 326-image one.
  The committed window is truncated at its newest 16,000, so the 84 is a floor; the exploratory run reached further back and saw more there *(uncommitted, so no count is quoted)*.
...
**The 2026-08-26 UTC window holds nothing** (committed: 0 images, 1 request); the "August 26" batch is stamped on **08-27** UTC — the committed window's newest 16,000 span 06:28–06:59 UTC, and the exploratory run found it starting at 05:54 *(uncommitted)*.
...
**Flagged, not resolved: those two batches read as late-evening local captures** — the committed spans are 23:28–23:59 PDT (22:54 at the start *(uncommitted)*) and 21:52–00:09 CDT — while the September batch reads as 15:40–17:00 PDT.
